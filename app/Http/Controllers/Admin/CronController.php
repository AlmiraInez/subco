...
class CronController extends Controller
{
    ...
    public function generate_allowance()
    {
        ...
        if (date('d', strtotime(date('Y-m-d'))) > $cut_off) {
            $month = date('m', strtotime($after_month));
            $year = date('Y', strtotime($after_year));
        ...
        foreach ($employeeallowances as $employeeallowance) {
            $cek = EmployeeAllowance::where(['employee_id' => $employeeallowance->employee_id, 'allowance_id' => $employeeallowance->allowance_id, 'month' => $month, 'year' => $year])->first();
            if (!$cek) {
        ...
        foreach ($employees as $employee) {
            ...
            if ($workgroup) {
                ...
                foreach ($workgroupallowances as $workgroupallowance) {
                    $cek = EmployeeAllowance::where(['employee_id' => $employee->id, 'allowance_id' => $workgroupallowance->allowance_id, 'month' => $month, 'year' => $year])->first();
                    if (!$cek) {
    ...
    public function generateOvertimeApprove()
    {
        $attendances = Attendance::where('status', 1)->whereMonth('attendance_date', '01')->get();
        foreach ($attendances as $key => $attendance) {
            $employee = Employee::find($attendance->employee_id);
            $overtimeScheme = OvertimeScheme::find($attendance->overtime_scheme_id);
